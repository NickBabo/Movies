...
final class BBRequestConfigurator {

    private let baseURL: URL
    weak var delegate: BBNetworkTokenDelegate?

    init(baseURL: URL, delegate: BBNetworkTokenDelegate?) {
        self.baseURL = baseURL
        self.delegate = delegate
    }

    func configure(_ service: BBServiceProtocol) -> URLRequest {
    ...
    private func configureQueryParameters(_ queryParameters: [String: String]?,
                                          to request: inout URLRequest) {
